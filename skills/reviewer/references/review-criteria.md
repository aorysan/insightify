...
1. **Accuracy**: Compare `[OUT_DIR]/docs/*` against `[OUT_DIR]/.insightify/knowledge/*`. All claims must match knowledge facts.
2. **Completeness**: Compare `[OUT_DIR]/docs/*` against `[OUT_DIR]/.insightify/plan.md`. All planned sections must be present.
3. **Consistency**: Terminology, tone, formatting must be uniform across all `[OUT_DIR]/docs/*`.

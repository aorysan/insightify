...
describe('Build Templates', () => {
  test('vitepress config template is valid JavaScript structure', () => {
    const configTpl = fs.readFileSync(path.join(__dirname, '../skills/builder/templates/vitepress-config.js'), 'utf8');
  ...
  test('index template contains hero frontmatter', () => {
    const indexTpl = fs.readFileSync(path.join(__dirname, '../skills/builder/templates/index-template.md'), 'utf8');
  ...
  test('sidebar template transforms plan pages into VitePress sidebar items', () => {
    const buildSidebar = require('../skills/builder/templates/sidebar-template.js');
    const pages = [
